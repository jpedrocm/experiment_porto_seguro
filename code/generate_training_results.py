...
if __name__ == "__main__":
	...
	DataHelper.remove_small_variance_cols(feats, inplace=True)
	DataHelper.remove_high_correlation_cols(feats, inplace=True)
	DataHelper.remove_high_nan_rate_cols(feats, inplace=True)

	for train_idxs, val_idxs in ConfigHelper.k_fold_cv(labels):
		train_X = DataHelper.select_rows(feats, train_idxs)
		train_y = DataHelper.select_rows(labels, train_idxs)
		val_X = DataHelper.select_rows(feats, val_idxs)
		val_y = DataHelper.select_rows(labels, val_idxs)
